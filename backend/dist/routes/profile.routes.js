 ...
 */
function calculateProfileStrength(dashboardJson) {
    ...
    if (profile?.testScores) {
        if (profile.testScores.gre || profile.testScores.gmat)
            score += 10;
        if (profile.testScores.toefl || profile.testScores.ielts)
            score += 10;
    }
...
router.post('/:userId/complete-item', async (req, res) => {
    try {
        ...
        const dashboardJson = snapshot.dashboardJson;
        let updates = {};
