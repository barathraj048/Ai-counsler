...
import { getDashboard, toggleTodo, createTodo, deleteTodo, getTodos, aiCounselor, discoverUniversities, // ← ADD THIS
shortlistUniversities, } from '../controllers/dashboard.controller.js';
const router = Router();
...
router.post('/universities/discover', async (req, res) => {
    try {
        ...
        const created = await Promise.all(universities.map(async (uni) => {
...
router.delete('/universities/:universityId/shortlist', async (req, res) => {
    try {
        ...
        const { userId } = req.query;
        const university = await prisma.university.findFirst({
            where: { id: universityId, userId: userId },
        });
        if (!university) {
            return res.status(404).json({ error: 'University not found' });
        }
