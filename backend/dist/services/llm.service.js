 ...
 */
export async function generateImprovementTasks(userProfile) {
    try {
 ...
 */
function getDefaultImprovementTasks(userProfile) {
    const tasks = [];
    // GPA Check
    if (!userProfile?.gpa || userProfile.gpa < 3.0) {
        tasks.push({
            ...
            description: 'A strong GPA (3.5+) significantly boosts your profile for competitive programs. Consider retaking key courses or focusing on upcoming semester performance.',
            impact: 15,
            category: 'academic',
    ...
    // Test Scores Check
    const hasTestScores = userProfile?.testScores?.gre ||
    ...
    if (!hasTestScores) {
        tasks.push({
            ...
            description: 'GRE/GMAT and English proficiency tests (TOEFL/IELTS) are required by most universities. Strong scores can offset other weaknesses in your application.',
            impact: 20,
            category: 'tests',
    ...
    if (!userProfile?.workExperience || userProfile.workExperience.length === 0) {
        tasks.push({
            ...
            description: 'Work experience, internships, or research projects demonstrate practical skills and commitment to your field. Quality matters more than quantity.',
            impact: 15,
            category: 'experience',
    ...
    if (!userProfile?.sop) {
        tasks.push({
            ...
            description: 'Your SOP is your chance to tell your unique story. It should connect your background, goals, and why this program is the perfect fit.',
            impact: 12,
            category: 'documents',
    ...
    if (!userProfile?.extracurriculars || userProfile.extracurriculars.length === 0) {
        tasks.push({
            ...
            description: 'Extracurriculars, volunteer work, and leadership roles demonstrate well-roundedness and soft skills valued by universities.',
            impact: 8,
            category: 'experience',
